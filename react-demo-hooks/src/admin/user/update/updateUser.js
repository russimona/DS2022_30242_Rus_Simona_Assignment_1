...
const UpdateUser = () => {
  ...
  const editUserHandler = () => {
    console.log({name : name, email: email, password: password, surname: surname});
    return editUser({id: editUid, name : name, email: email, password: password, surname: surname}, (result, status, err) => {
      if (result !== null && (status === 200 || status === 201)) {
        alert("Successfully edited person with id: " + result);
        
      } else {
        alert(
          "Error trying to edit user."
        );
      }
    });
  };
